...
class ParseMarkdown:
    ...
    def parse_markdown(self) -> dict:
        ...
        def line_is_heading(line):
            return line[0] == "#"
        def line_starts_with_backticks(line):
        ...
        # code block parsing ###################################################
        is_code_block = False
        code_block_fence_length = 0  # number of backticks in opening fence
        code_block_language = ""
        code_block = []
        ...
        for line in self.raw_text.splitlines():
            ...
            if line_starts_with_backticks(line):
                if not is_code_block:
                    # start ```<language> of codeblock
                    is_code_block = True
                    code_block_fence_length = self._count_fence_backticks(line)

                    code_block_language = (
                        self.get_codeblock_language(
                            line, code_block_fence_length, section_tracker.index
                        ) or ""
                    )
                    ...
                    # only close if closing fence has >= opening fence backticks
                    closing_fence_length = self._count_fence_backticks(line)
                    if closing_fence_length >= code_block_fence_length:
                        is_code_block = False
                        self.parse_codeblock_end(self, code_block, section_tracker.index)
                        section_tracker.current.append(code_block)
                    else:
                        # nested ``` inside block — treat as content
